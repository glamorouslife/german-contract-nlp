...
training_args = TrainingArguments(
    ...
    num_train_epochs=5,
    weight_decay=0.01,
    eval_strategy="epoch",          # ← NEW name, fixed ✅
    save_strategy="epoch",
...
class CustomTrainer(Trainer):
    def compute_loss(self, model, inputs, return_outputs=False):
        # Use global DEVICE; Trainer already handles device placement
        labels = inputs.get("labels")
        # forward pass
        outputs = model(**inputs)
        logits = outputs.get("logits")
        # compute custom loss with weight tensor on the correct device
        loss_fct = torch.nn.CrossEntropyLoss(weight=torch.tensor([1.0, 100.0], device=DEVICE))
